...
ad9528Device_t ad9528DevicePrivCfg = {0};

Device_t  ad9528Device = {.deviceName = "AD9528",
		                  .spi = NULL,
						  .gpio = NULL,
						  ._desired_time_to_elapse_10ms = 0,
						  .DevicePrivCfg = &ad9528DevicePrivCfg
};
					...
					uint32_t outputDeviceClock_Hz)
{
	...
	device->pll1Settings->refA_bufferCtrl =
			DISABLED;//DIFFERENTIAL;	   //DISABLED = 0, SINGLE_ENDED, NEG_SINGLE_ENDED, DIFFERENTIAL
	...
	device->pll1Settings->vcxoBufferCtrl  =
			DIFFERENTIAL;//NEG_SINGLE_ENDED;   //DISABLED = 0, SINGLE_ENDED, NEG_SINGLE_ENDED, DIFFERENTIAL
	...
	device->outputSettings->outPowerDown = (uint16_t)(
			    ~0x0BFF);//~0x300A);	 //enable outputs 13,12,3 and 1
	...
	for (i=0; i<14; i++) {
		...
		//if((i == 4) || (i == )){
			//device->outputSettings->outChannelDiv[i] = 4;
		//}
	...
	//device->outputSettings->outSource[0] = SYSREF;
	device->outputSettings->outSource[1] = SYSREF;


	device->sysrefSettings->sysrefDivide =
		1; //SYSREF frequency = VCXO / 2 / sysrefDivde
	...
	device->sysrefSettings->sysrefRequestMethod =SPI;//PIN;		  //SPI=0,PIN=1
			      ...
			      *device)   //writes all registers to desired values, leaves clock outputs off, SYSREF output disabled.
{
	...
	AD9528_spiReadByte(device, AD9528_ADDR_ADI_SPI_CONFIG_A, &readdata);
	printf("AD9528_ADDR_ADI_SPI_CONFIG_A is : 0x%02x\n", readdata);
	mdelay(100);
	AD9528_spiReadByte(device, AD9528_ADDR_CHIP_TYPE, &readdata);
	printf("AD9528 chip type is : 0x%02x\n", readdata);
	mdelay(100);
	AD9528_spiReadByte(device, AD9528_ADDR_PRODUCT_ID, &readdata);
	printf("AD9528 product ID is : 0x%02x\n", readdata);
	mdelay(100);
	AD9528_spiReadByte(device, AD9528_ADDR_PART_VERSION, &readdata);
	printf("AD9528 PART VERSION is : 0x%02x\n", readdata);
	mdelay(100);
	AD9528_spiReadByte(device, AD9528_ADDR_SPI_VERSION, &readdata);
	printf("AD9528 SPI VERSION is : 0x%02x\n", readdata);
	mdelay(100);
	AD9528_spiReadByte(device, AD9528_ADDR_VENDOR_ID_L, &readdata);
	printf("AD9528 VENDOR ID L is : 0x%02x\n", readdata);
	mdelay(100);
	AD9528_spiReadByte(device, AD9528_ADDR_VENDOR_ID_H, &readdata);
	printf("AD9528 VENDOR ID H is : 0x%02x\n", readdata);
	mdelay(100);
	...
	do {
		...
		if(AD9528_hasTimeoutExpired() > 0) {
			printf("reg508 :0x%02x\n",reg508);
			AD9528_spiReadByte(device, AD9528_ADDR_PLL2_LF_CTRL1, &readdata);
			printf("AD9528 PLL2_LF_CTRL1 is : 0x%02x\n", readdata);
