...
//#pragma DATA_SECTION (dataBuff, ".CORE0buf")
...
void usageTsk(UArg arg0, UArg arg1)
{
    ...
    if (corenum == SYSINIT)
    {
        ...
        qmssInitConfig.linkingRAM0Base = 0;//l2_global_address ((uint32_t) linkingRAM0);
        qmssInitConfig.linkingRAM0Size = 0;//(NUM_HOST_DESC + NUM_MONOLITHIC_DESC) * NUM_CORES;
    ...
    if(corenum == SYSINIT){
        ...
        rxFlowCfg.rx_psinfo_present = 1; //set 0
        ...
        for(i = 0; i< 1; i++)
        {
            ...
            System_printf("Core 0 Get Queue %d Entry Count is: %d\n",rxQueHnd, result);
            ...
            System_printf("*******destDataPtr 1 is : 0x%02x, addr: 0x%08x\n",destDataPtr[1], destDataPtr);
            System_printf("*******dataBuffPtr 1 is : 0x%02x, addr: 0x%08x\n",dataBuffPtr[0], dataBuffPtr);
            //queInfo = Cppi_getReturnQueue (Cppi_DescType_HOST, rxPkt);
            Qmss_queuePushDesc (freeQueHnd, (uint32_t *) rxPkt);
            System_printf("******Get %d desc!!!!!\n",i+1);
        }
        end_time = CSL_chipReadReg(CSL_CHIP_TSCL);
        System_printf("*******time is : %d\n",end_time - start_time);
        ...
        {
            //open tx queue
            if ((txQueHnd = Qmss_queueOpen (Qmss_QueueType_LOW_PRIORITY_QUEUE, 800, &isAllocated)) < 0)
            {
                System_printf ("Error Core %d : Opening Queue Number\n", corenum);
                return;
            }
            ...
            for(i = 0; i< 1; i++)//while(() != NULL)
            {
                ...
                System_printf ("Core %d : srcTagLo is : %d \n", corenum, Tag.srcTagLo);
                Qmss_queuePushDescSize (txQueHnd, (Ptr) hostDescPtr, SIZE_HOST_DESC);
                System_printf ("Core %d : Qmss_queuePushDescSize OK\n", corenum);
            ...
            System_printf("*******srcData 1 is : 0x%02x, addr: 0x%08x\n",srcData[0], srcData);
            System_printf("*******dataBuff 1 is : 0x%02x, addr: 0x%08x\n",dataBuff[0], dataBuff);
        }
        }
    System_flush();
    while(1);
