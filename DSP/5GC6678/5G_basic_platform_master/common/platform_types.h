...
/* boolean_t is also defined in commonDef.h,
...
typedef int32_t            rlc_sn_t;
typedef uint32_t           rlc_usn_t;
...
typedef struct protocol_ctxt_s {
	...
	uint8_t     slot;     /*!< \brief  valid for UE indicating the index of connected eNB(s)      */
...
#if 1
#define UE_MODULE_ID_TO_INSTANCE( uEID ) uEID
#else
...
#if 1
#define MODULE_ID_TO_INSTANCE(uEID, iNSTANCE, eNB_fLAG) \
    if(eNB_fLAG == ENB_FLAG_YES) \
        iNSTANCE = ENB_MODULE_ID_TO_INSTANCE(uEID); \
    else \
        iNSTANCE = UE_MODULE_ID_TO_INSTANCE(uEID)
#else
...
#define PROTOCOL_CTXT_SET_BY_UE_ID(Ctxt_Pp, mODULE_iD, rNTI, fRAME, sUBfRAME, sLOT) \
...
#if 1
#define PROTOCOL_CTXT_SET_BY_MODULE_ID(Ctxt_Pp, mODULE_iD, rNTI, fRAME, sUBfRAME, sLOT) \
    (Ctxt_Pp)->module_id = mODULE_iD; \
    (Ctxt_Pp)->rnti      = rNTI; \
    (Ctxt_Pp)->frame     = fRAME; \
    (Ctxt_Pp)->subframe  = sUBfRAME; \
