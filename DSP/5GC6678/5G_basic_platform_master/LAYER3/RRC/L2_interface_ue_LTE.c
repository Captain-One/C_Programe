...
//--------------------------------------------------------------------------
{
  printf("[eNB %d] mac_rrc_data_req to SRB ID=%d\n",Mod_idP,Srb_id);
#if 0//(RRC_VERSION < MAKE_VERSION(14, 0, 0))
  printf("[UE %d] Frame %d Filling SL DISCOVERY SRB_ID %d\n",Mod_idP,frameP,Srb_id);
  printf("[UE %d] Frame %d buffer_pP status %d,\n",Mod_idP,frameP, UE_rrc_inst[Mod_idP].SL_Discovery[eNB_index].Tx_buffer.payload_size);
  ...
  if (Srb_id  == SL_DISCOVERY && UE_rrc_inst[Mod_idP].SL_Discovery[eNB_index].Tx_buffer.payload_size > 0) {
    ...
    printf("[UE %d] Sending SL_Discovery, size %d bytes\n",Mod_idP,Ret_size);
    UE_rrc_inst[Mod_idP].SL_Discovery[eNB_index].Tx_buffer.payload_size = 0;
    return(Ret_size);
  }

#endif
  printf("[UE %d] Frame %d Filling CCCH SRB_ID %d\n",Mod_idP,frameP,Srb_id);
...
//--------------------------------------------------------------------------
{
  ...
  int si_window;
   */
  PROTOCOL_CTXT_SET_BY_MODULE_ID(&ctxt, module_idP, rntiP, frameP, sub_frameP,eNB_indexP);

#if 0//(RRC_VERSION < MAKE_VERSION(14, 0, 0))
   if(srb_idP == BCCH_SI_MBMS) {
      printf("[UE %d] Received SDU for BCCH on MBMS SRB %d from eNB %d\n",module_idP,srb_idP,eNB_indexP);
  ...
  if(srb_idP == BCCH) {
    printf("[UE %d] Received SDU for BCCH on SRB %d from eNB %d\n",module_idP,srb_idP,eNB_indexP);
#if defined(ENABLE_ITTI)
    {
      MessageDef *message_p;
...
//      rrc_ue_decode_ccch(&ctxt, Srb_info, eNB_indexP);
  ...
  if ((srb_idP & RAB_OFFSET) == MCCH) {
    printf("[UE %d] Frame %d: Received SDU on MBSFN sync area %d for MCCH on SRB %d from eNB %d\n",
  ...
  if(srb_idP == SL_DISCOVERY) {
    printf("[UE %d] Received SDU (%d bytes) for SL_DISCOVERY on SRB %d from eNB %d\n",module_idP, sdu_lenP, srb_idP,eNB_indexP);
...
//------------------------------------------------------------------------------
void
rrc_data_ind_ue(const protocol_ctxt_t *const ctxt_pP, const rb_id_t Srb_id, const sdu_size_t sdu_sizeP, const uint8_t *const buffer_pP)
//------------------------------------------------------------------------------
{
  rb_id_t    DCCH_index = Srb_id;
  printf("[UE %x] Frame %d: received a DCCH %d message on SRB %d with Size %d from eNB 0\n",
        ctxt_pP->module_id, ctxt_pP->frame, DCCH_index,Srb_id,sdu_sizeP);
#if defined(ENABLE_ITTI)
  {
    MessageDef *message_p;
    // Uses a new buffer to avoid issue with PDCP buffer content that could be changed by PDCP (asynchronous message handling).
...
//-------------------------------------------------------------------------------------------//
{
	if (UE_rrc_inst[Mod_idP].Info[eNB_index].N310_cnt>10)
		printf("[UE %d] Frame %d: OUT OF SYNC FROM eNB %d (T310 active %d : T310 %d, N310 %d, N311 %d)\n ",
			Mod_idP,frameP,eNB_index, UE_rrc_inst[Mod_idP].Info[eNB_index].T300_active,
			UE_rrc_inst[Mod_idP].Info[eNB_index].T310_cnt, UE_rrc_inst[Mod_idP].Info[eNB_index].N310_cnt,
			UE_rrc_inst[Mod_idP].Info[eNB_index].N311_cnt);
...
//------------------------------------------------------------------------------
int mac_UE_get_rrc_status(const module_id_t Mod_idP, const uint8_t indexP)
//------------------------------------------------------------------------------
{
	if (UE_rrc_inst)
