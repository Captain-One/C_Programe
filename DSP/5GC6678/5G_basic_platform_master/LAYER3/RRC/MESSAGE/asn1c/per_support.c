...
static int
per__ulong_range(unsigned long lb, unsigned long ub, unsigned long *range_r) {
    unsigned long bounds_range;
    if((ub < 0) == (lb < 0)) {
        bounds_range = ub - lb;
    } else {
...
int
per_ulong_range_rebase(unsigned long v, unsigned long lb, unsigned long ub, unsigned long *output) {
    unsigned long range;

    assert(lb <= ub);

    if(v < lb || v > ub || per__ulong_range(lb, ub, &range) < 0) {
        /* Range error. */
        return -1;
    }
