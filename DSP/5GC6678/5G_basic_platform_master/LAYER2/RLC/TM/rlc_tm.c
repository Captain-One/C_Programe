							...
							const sdu_size_t length_in_bitsP)
{
	...
	if ((rlc_pP->output_sdu_in_construction)) {
		...
		printf("[RLC_TM %p][SEND_SDU] ERROR  OUTPUT SDU IS NULL\n", rlc_pP);
	}
}
//-----------------------------------------------------------------------------
void rlc_tm_no_segment(const protocol_ctxt_t* const ctxt_pP, rlc_tm_entity_t *const rlc_pP)
{
	...
	while ((rlc_pP->input_sdus[rlc_pP->current_sdu_index]) && (nb_pdu_to_transmit > 0)) {
		...
		if (!(pdu_p = get_free_mem_block (((rlc_pP->rlc_pdu_size + 7) >> 3) + sizeof (struct rlc_tm_tx_data_pdu_struct) + GUARD_CRC_LIH_SIZE, __func__))) {
			printf(PROTOCOL_RLC_TM_CTXT_FMT"[SEGMENT] ERROR COULD NOT GET NEW PDU, EXIT\n",
					PROTOCOL_RLC_TM_CTXT_ARGS(ctxt_pP, rlc_pP));
			return;
		}
...
//-----------------------------------------------------------------------------
void
rlc_tm_rx(const protocol_ctxt_t* const ctxt_pP, void * const argP, struct mac_data_ind data_indP)
{
...
//-----------------------------------------------------------------------------
struct mac_status_resp rlc_tm_mac_status_indication(
...
//-----------------------------------------------------------------------------
struct mac_data_req rlc_tm_mac_data_request(const protocol_ctxt_t* const  ctxt_pP, void * const rlc_pP)
{
	...
	if(data_req.data.nb_elements > 0) {
		printf(PROTOCOL_RLC_TM_CTXT_FMT" MAC_DATA_REQUEST %d TBs\n",
				PROTOCOL_RLC_TM_CTXT_ARGS(ctxt_pP, rlc_p), data_req.data.nb_elements);
	}
...
//-----------------------------------------------------------------------------
void rlc_tm_data_req (const protocol_ctxt_t* const  ctxt_pP, void *const rlc_pP, mem_block_t *const sdu_pP)
{
	rlc_tm_entity_t *rlc_p = (rlc_tm_entity_t *) rlc_pP;

#if DEBUG_RLC_TM_DATA_REQUEST
	printf(PROTOCOL_RLC_TM_CTXT_FMT" RLC_TM_DATA_REQ size %d Bytes, BO %ld , NB SDU %d current_sdu_index=%d next_sdu_index=%d\n",
			PROTOCOL_RLC_TM_CTXT_ARGS(ctxt_pP, rlc_p), ((struct rlc_um_data_req *) (sdu_pP->data))->data_size,
			rlc_p->buffer_occupancy, rlc_p->nb_sdu, rlc_p->current_sdu_index, rlc_p->next_sdu_index);
#endif
