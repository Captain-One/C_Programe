							...
							const logical_chan_id_t chan_idP)
{
	...
	if (h_rc == HASH_TABLE_OK) {
		...
		printf(" CONFIG_REQ (is_uplink_downlink=%d) RB %u\n", config_tmP->is_uplink_downlink, rb_idP);
		...
		printf(" CONFIG_REQ RB %u RLC NOT FOUND\n", rb_idP);
	}
}

//-----------------------------------------------------------------------------
void rlc_tm_init (const protocol_ctxt_t* const  ctxt_pP, rlc_tm_entity_t * const rlcP)
{
...
//-----------------------------------------------------------------------------
void rlc_tm_reset_state_variables (const protocol_ctxt_t* const  ctxt_pP, struct rlc_tm_entity * const rlcP)
{
	rlcP->output_sdu_size_to_write = 0;
	rlcP->buffer_occupancy = 0;
	rlcP->nb_sdu = 0;
...
//-----------------------------------------------------------------------------
void rlc_tm_set_debug_infos(const protocol_ctxt_t* const  ctxt_pP,
									rlc_tm_entity_t * const rlcP,
									const srb_flag_t  srb_flagP,
									const rb_id_t     rb_idP,
									const logical_chan_id_t chan_idP)
