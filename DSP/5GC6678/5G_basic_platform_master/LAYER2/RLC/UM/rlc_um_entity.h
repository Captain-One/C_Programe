...
//	 PROTOCOL VARIABLES
//	-----------------------------
	rlc_usn_t TX_Next; /* This state variable holds the value of the SN to be assigned for the next newly generated UMD PDU with segment. */
	rlc_usn_t RX_Next;
	rlc_usn_t RX_Next_Reassembly; /* This state variable holds the value of the earliest SN that is still considered for reassembly. */
	rlc_usn_t RX_Timer_Trigger; /* This state variable holds the value of the SN following the SN which triggered t-Reassembly. */
	rlc_usn_t RX_Next_Highest; /* This state variable holds the value of the SN following the SN of the UMD PDU with the highest SN among received UMD PDUs. It serves as the higher edge of the reassembly window.  */
//	-----------------------------
//	 TIMERS
//	-----------------------------
	struct rlc_um_timer_s  t_reassembly; /*!< \brief This timer is used by the receiving side of an AM RLC entity and receiving UM RLC entity in order to detect loss of RLC PDUs at lower layer (see sub clauses 5.1.2.2 and 5.1.3.2). If t-Reordering is running, t-Reordering shall not be started additionally, i.e. only one t-Reordering per RLC entity is running at a given time. */
//	*****************************************************************************
...
//	pthread_spinlock_t lock_input_sdus;
	...
	list2_t               input_sdus;                /*!< \brief Input SDU buffer (for SDUs coming from upper layers). Should be accessed as an array. */
	rlc_buffer_occupancy_t buffer_occupancy;          /*!< \brief Number of bytes contained in input_sdus buffer. RLC SDU输入缓冲区数据量（字节数）*/
	int32_t             nb_bytes_requested_by_mac; /*!< \brief Number of bytes requested by lower layer for next transmission. */
	list2_t               pdus_to_mac_layer;         /*!< \brief PDUs buffered for transmission to MAC layer. */
//	*****************************************************************************
//	 RECEIVER
//	*****************************************************************************
...
//	pthread_spinlock_t lock_dar_buffer;
	...
	list2_t            pdus_from_mac_layer;            /*!< \brief Not Used. */
