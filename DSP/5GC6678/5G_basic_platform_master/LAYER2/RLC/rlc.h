...
#define  RLC_MAX_LC  ((max_val_DRB_Identity+1)* MAX_MOBILES)
...
#define RLC_COLL_KEY_VALUE(mODULE_iD, rNTI, rB_iD, iS_sRB) \
    ((hash_key_t)(rNTI)  			| \
...
#define RLC_COLL_KEY_LCID_VALUE(mODULE_iD, rNTI, lC_iD, iS_sRB) \
		((hash_key_t)(rNTI)  			| \
		(((hash_key_t)(iS_sRB))	<< 16) | \
		(((hash_key_t)(mODULE_iD)) << 17) | \
		(((hash_key_t)(lC_iD)) 	<< 21) | \
...
*/
tbs_size_t            mac_rlc_serialize_tb   (const logical_chan_id_t, char*, list_t);
...
* \return     A status about the processing, OK or error code.
*/
//tbs_size_t            mac_rlc_data_req     (const uint8_t, const rnti_t, const frame_t, const  eNB_flag_t, const  MBMS_flag_t, logical_chan_id_t, const tb_size_t,char*
...
*/
mac_rlc_status_resp_t mac_rlc_status_ind   (const module_id_t, const rnti_t, const eNB_index_t, const frame_t, const sub_frame_t, const slot_t, const  eNB_flag_t, const  MBMS_flag_t, logical_chan_id_t, tb_size_t
#if 0//(RRC_VERSION < MAKE_VERSION(15, 0, 0))
                                                           ,const uint32_t sourceL2Id
                                                           ,const uint32_t destinationL2Id
#endif
