...
//Assertion(eNB)_PRAN_DesignDocument_annex No.764
     printf(" SEND SDU REQUESTED %d bytes\n",
//             PROTOCOL_RLC_AM_CTXT_ARGS(ctxt_pP,rlc_pP),
             rlc_pP->output_sdu_size_to_write);
/*
      AssertFatal(3==4,
  ...
  boolean_t free_rlc_pdu)
{
	...
	while(pdu_info_p->sn == sn)
	{
		((rlc_am_rx_pdu_management_t*)(cursor_p->data))->segment_reassembled = RLC_AM_RX_PDU_SEGMENT_REASSEMBLED;

		memcpy(rlc_pP->output_sdu_in_construction->data + pdu_info_p->so, pdu_info_p->payload, pdu_info_p->payload_size);
