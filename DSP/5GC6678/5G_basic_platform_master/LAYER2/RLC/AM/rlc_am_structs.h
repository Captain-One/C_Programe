...
typedef struct rlc_am_tx_sdu_management {
	...
	uint16_t					nack_so_start[RLC_AM_MAX_HOLES_REPORT_PER_PDU]; /*!< \brief Lowest NACK start segment offset, must be set to 0 if global NACK. */
	uint16_t					nack_so_end[RLC_AM_MAX_HOLES_REPORT_PER_PDU];  /*!< \brief Highest NACK stop segment offset, must be set to data_size - 1 if global NACK */
	...
	rlc_sn_t					sn;	/* associate SN with the RLC SDU */
...
typedef struct rlc_am_tx_data_pdu_management {
  ...
  uint16_t       hole_so_start  [RLC_AM_MAX_HOLES_REPORT_PER_PDU]; /*!< \brief Array containing the start segment offsets for marking a hole (negative acknowledged area) in the PDU. */
  uint16_t       hole_so_stop   [RLC_AM_MAX_HOLES_REPORT_PER_PDU]; /*!< \brief Array containing the stop segment offsets for marking a hole (negative acknowledged area) in the PDU. */
  ...
  int32_t         sn;                                /*!< \brief Sequence number of the PDU. */
  sdu_size_t       nack_so_start[RLC_AM_MAX_HOLES_REPORT_PER_PDU]; /*!< \brief Lowest NACK start segment offset, must be set to 0 if global NACK. */
  sdu_size_t       nack_so_stop[RLC_AM_MAX_HOLES_REPORT_PER_PDU];  /*!< \brief Highest NACK stop segment offset, must be set to data_size - 1 if global NACK */
...
*/
#if 1
typedef struct nack12_s {
	union {
		struct {
		  uint16_t nack_sn:12;  /*!< \brief NACK sequence number field. */
		  uint16_t e1:1;        /*!< \brief Extension bit 1 field. */
		  uint16_t e2:1;        /*!< \brief Extension bit 2 field. */
		  uint16_t e3:1;        /*!< \brief Extension bit 3 field. */
		  uint16_t r:1;
		} nack_st;
		uint16_t u16_nack;
	} nack_u;
  uint16_t so_start; /*!< \brief Optional SOstart field (depending on e2 bit). */
  uint16_t so_end;   /*!< \brief Optional SOend field (depending on e2 bit). */
  uint16_t nack_range:8;
  uint16_t r:8;
} nack12_t;

typedef struct nack18_s {
	union {
		struct {
		  uint32_t nack_sn:18;  /*!< \brief NACK sequence number field. */
		  uint32_t e1:1;        /*!< \brief Extension bit 1 field. */
		  uint32_t e2:1;        /*!< \brief Extension bit 2 field. */
		  uint32_t e3:1;        /*!< \brief Extension bit 3 field. */
		  uint32_t r:3;
		  uint32_t dummy:8;
		} nack_st;
		uint32_t u32_nack;
	} nack_u;
  uint32_t so_start:16; /*!< \brief Optional SOstart field (depending on e2 bit). */
  uint32_t so_end:16;   /*!< \brief Optional SOend field (depending on e2 bit). */
  uint8_t nack_range:8;
} nack18_t;

//typedef union nack_s {
//	nack12_t nack12;
//	nack18_t nack18;
//} nack_t;
#else
...
typedef struct rlc_am_control_ack12_s {
	uint32_t	d_c:1;       /*!< \brief Data/Control field. */
	uint32_t	cpt:3;       /*!< \brief Control PDU type. */
	uint32_t	ack_sn:12;
	uint32_t	E1:1;
	uint32_t	R:7;
	uint32_t	dummy:8;
} rlc_am_control_ack12_t;

typedef struct rlc_am_control_ack18_s {
	uint32_t	d_c:1;       /*!< \brief Data/Control field. */
	uint32_t	cpt:3;       /*!< \brief Control PDU type. */
	uint32_t	ack_sn:18;
	uint32_t	E1:1;
	uint32_t	R:1;
	uint32_t	dummy:8;
} rlc_am_control_ack18_t;

typedef union rlc_am_control_ack_u {
	rlc_am_control_ack12_t ack12;
	rlc_am_control_ack18_t ack18;
	uint32_t u32_ack;
} rlc_am_control_ack_t;

typedef union rlc_am_control_nack12 {
	struct {
		uint16_t nack_sn:12;
		uint16_t e1:1;
		uint16_t e2:1;
	...
	uint16_t u16_nack12;
} rlc_am_control_nack12_t;

typedef union rlc_am_control_nack18 {
	struct {
	  uint32_t nack_sn:18;  /*!< \brief NACK sequence number field. */
	  uint32_t e1:1;        /*!< \brief Extension bit 1 field. */
	  uint32_t e2:1;        /*!< \brief Extension bit 2 field. */
	...
	uint32_t u32_nack;
} rlc_am_control_nack18_t;

typedef struct nack_s {
	union {
		rlc_am_control_nack12_t nack12;
		rlc_am_control_nack18_t nack18;
		uint32_t u32_nack;
	} nack_u;
	uint16_t so_start;
	uint16_t so_end;
...
typedef struct rlc_am_control_nack {
	uint8_t e1:1;
	uint8_t e2:1;
	uint8_t e3:1;
	uint8_t retx:1;
	...
	uint16_t so_end;
	uint32_t nack_sn;
} rlc_am_control_nack_t;

typedef union rlc_am_pdu_ctrl_header {
	struct {
	  uint8_t  d_c:1;  /*!< \brief Data/Control field. */
	  uint8_t  cpt:3;    /*!< \brief Polling bit field. */
	  uint8_t	 snh:4;
	} header_st;
	uint8_t header8;
} __attribute__((__packed__)) rlc_am_pdu_ctrl_header_t ;

#if 1
typedef struct rlc_am_control_pdu_info_s {
	rlc_am_pdu_ctrl_header_t header;
	int8_t e1;
	int32_t ack_sn;
	uint16_t      num_nack;    /*!< \brief Number of nack_sn_t fields in nack_list[]. */
	rlc_am_control_nack_t  nack_list[RLC_AM_MAX_NACK_IN_STATUS_PDU]; /*!< \brief List of NACK_SN fields. */
} rlc_am_control_pdu_info_t;
#else
typedef struct rlc_am_control_pdu_info {
	rlc_am_control_ack_t ack;
	uint16_t      num_nack;    /*!< \brief Number of nack_sn_t fields in nack_list[]. */
	nack_t  nack_list[RLC_AM_MAX_NACK_IN_STATUS_PDU]; /*!< \brief List of NACK_SN fields. */
} rlc_am_control_pdu_info_t ;
//#else
typedef struct rlc_am_control_pdu_info {
  uint16_t      d_c:1;       /*!< \brief Data/Control field. */
...
* @{
*/
typedef union rlc_am_pdu_header {
	struct {
	  uint8_t  d_c:1;  /*!< \brief Data/Control field. */
	  uint8_t  p:1;    /*!< \brief Polling bit field. */
	  uint8_t  si:2;   /*!< \brief Framing Info field. */
	  uint8_t	 R:4;
	} header_st;
	uint8_t header8;
} __attribute__((__packed__)) rlc_am_pdu_header_t;
/** @} */
...
typedef struct rlc_am_pdu_info {
  rlc_am_pdu_header_t header;
  uint32_t  sn;  /*!< \brief Sequence Number field. */
  uint16_t  so;  /*!< \brief Segment Offset field. */
...
typedef struct rlc_am_pdu12_header {
	uint8_t	d_c:1;  /*!< \brief Data/Control field. */
	uint8_t	p:1;    /*!< \brief Polling bit field. */
	uint8_t	si:2;   /*!< \brief Framing Info field. */
	uint8_t	snh4:4;
	...
	uint8_t	soh;
	uint8_t	sol;
} rlc_am_pdu12_header_t;

typedef struct rlc_am_pdu18_header {
	uint8_t	d_c:1;  /*!< \brief Data/Control field. */
	uint8_t	p:1;    /*!< \brief Polling bit field. */
	uint8_t	si:2;   /*!< \brief Framing Info field. */
	uint8_t	r:2;
	...
	uint8_t	soh;
	uint8_t	sol;
} rlc_am_pdu18_header_t;

typedef union rlc_am_header_u {
	rlc_am_pdu12_header_t h12;
	rlc_am_pdu18_header_t h18;
} rlc_am_header_t;
#else
typedef struct rlc_am_pdu_info {
  uint32_t  d_c:1;  /*!< \brief Data/Control field. */
