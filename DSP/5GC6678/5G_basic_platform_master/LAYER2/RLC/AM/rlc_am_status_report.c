	...
	rlc_am_control_pdu_info_t* const pdu_info_pP)
{
	...
	uint32_t offset = 0;
	uint8_t* byte_pos_p = header_pP;
	nack_t* nack_pP = NULL;
	uint16_t num_nack = 0;

  memset(pdu_info_pP, 0, sizeof (rlc_am_control_pdu_info_t));

//  pdu_info_pP->header = *byte_pos_p;
  pdu_info_pP->header.header8 = *header_pP;


  if (!pdu_info_pP->header.header_st.d_c)
  {
//	  控制PDU类型不为0,则返回错误
    if (pdu_info_pP->header.header_st.cpt != 0x00)
    {
      return -3;
    }
    ...
    {
    	pdu_info_pP->ack_sn = (((uint32_t)pdu_info_pP->header.header_st.snh) << 14) + (((uint16_t)(*(byte_pos_p + 1))) << 8) + ((*(byte_pos_p + 2)) >> 2) & 0xFC;
    	pdu_info_pP->e1 = ((*(header_pP + 2)) >> 1) & 0x01;
    }
    else // SN 12bit
    {
    	pdu_info_pP->ack_sn = (((uint16_t)pdu_info_pP->header.header_st.snh) << 8) + (*(byte_pos_p + 1));
    	pdu_info_pP->e1 = ((*(byte_pos_p + 2)) >> 7) & 0x01;
    }
...
//      uint8_t*        byte_pos_p      = &header_pP->b2;
    	do
    	{
    		nack_pP = byte_pos_p + offset;

//    		pdu_info_pP->nack_list[pdu_info_pP->num_nack].nack_u.u32_nack = nack_pP->nack_u->u32_nack;

    		if(rlc_pP->tx_sn_length == 18)
    		{
    			pdu_info_pP->nack_list[num_nack].nack_sn = nack_pP->nack_u.nack18.nack_st.nack_sn;
    			pdu_info_pP->nack_list[num_nack].e1 = nack_pP->nack_u.nack18.nack_st.e1;
    			pdu_info_pP->nack_list[num_nack].e2 = nack_pP->nack_u.nack18.nack_st.e2;
    			pdu_info_pP->nack_list[num_nack].e3 = nack_pP->nack_u.nack18.nack_st.e3;

    			offset += 3;

    			if(nack_pP->nack_u.nack18.nack_st.e2)
    			{
    				pdu_info_pP->nack_list[pdu_info_pP->num_nack].so_start = *((uint16_t*)(byte_pos_p + offset));
    				offset += 2;
    				pdu_info_pP->nack_list[pdu_info_pP->num_nack].so_end = *((uint16_t*)(byte_pos_p + offset));
    				offset += 2;
    			}

    			if(nack_pP->nack_u.nack18.nack_st.e3)
    			{
    				pdu_info_pP->nack_list[pdu_info_pP->num_nack].nack_range = *(byte_pos_p + offset);
    				offset += 1;
    			}
    		}
			...
			{
    			pdu_info_pP->nack_list[num_nack].nack_sn = nack_pP->nack_u.nack12.nack_st.nack_sn;
				pdu_info_pP->nack_list[num_nack].e1 = nack_pP->nack_u.nack12.nack_st.e1;
				pdu_info_pP->nack_list[num_nack].e2 = nack_pP->nack_u.nack12.nack_st.e2;
				pdu_info_pP->nack_list[num_nack].e3 = nack_pP->nack_u.nack12.nack_st.e3;
				offset += 2;

				if(nack_pP->nack_u.nack12.nack_st.e2)
				{
					pdu_info_pP->nack_list[pdu_info_pP->num_nack].so_start = *((uint16_t*)(byte_pos_p + offset));
					offset += 2;
					pdu_info_pP->nack_list[pdu_info_pP->num_nack].so_end = *((uint16_t*)(byte_pos_p + offset));
				...
				if(nack_pP->nack_u.nack12.nack_st.e3)
				{
					pdu_info_pP->nack_list[pdu_info_pP->num_nack].nack_range = *(byte_pos_p + offset);
					offset += 1;
				}
    		...
    		num_nack += 1;
    		pdu_info_pP->num_nack += num_nack;

    		if(offset > *total_size_pP)
    		{
    			return -1;
    		}
    	} while(((rlc_pP->tx_sn_length == 18) ? nack_pP->nack_u.nack18.nack_st.e1 : nack_pP->nack_u.nack12.nack_st.e1));

      *total_size_pP -= offset;
    }
    else
    {
      *total_size_pP = *total_size_pP - 3;
    }

    if (*total_size_pP != 0)
    {
      printf("[RLC_AM_GET_CONTROL_PDU_INFOS][SECOND]header_pP->b1=%d,header_pP->b2=%d\n", *(header_pP),*(header_pP + 1));
    }
...
)
{
  ...
  if (!pdu_info_pP->header.header_st.d_c) {
    printf("CONTROL PDU ACK SN %04d", pdu_info_pP->ack_sn);
...
//  rlc_sn_t		vt_a_new  = rlc_pP->vt_a;
	...
	uint32_t        sn_cursor;
	uint32_t		vt_a_new;
	uint32_t		sn_data_cnf;
	uint32_t        nack_sn,prev_nack_sn;
//  rlc_sn_t		sn_data_cnf;
//  rlc_sn_t        nack_sn,prev_nack_sn;
  sdu_size_t		data_cnf_so_stop = 0x7FFF;
  unsigned int nack_index;
  boolean_t status = TRUE;
  mem_block_t* segmentation_pdu_mbP;

  if (rlc_am_get_control_pdu_infos(rlc_pP, rlc_am_pdu_sn_10_p, tb_size_in_bytes_pP, &rlc_pP->control_pdu_info) >= 0)
  {

    rlc_am_tx_buffer_display(ctxt_pP, rlc_pP, " TX BUFFER BEFORE PROCESS OF STATUS PDU");
    printf(" RX CONTROL PDU TX_Next_Ack %04d TX_Next %04d POLL_SN %04d ACK_SN %04d\n",
...
//    3GPP 38322 F50 5.3.2	Retransmission
...
//    	-	set the P field according to sub clause 5.3.3.
    ...
    if(ack_sn >= rlc_pP->tx_sn_modulo || rlc_pP->control_pdu_info.num_nack >= RLC_AM_MAX_NACK_IN_STATUS_PDU)
    {
      printf(" illegal ack_sn %d, num_nack %d\n", ack_sn, rlc_pP->control_pdu_info.num_nack);
      return;
    }
...
//    if (RLC_AM_DIFF_SN(rlc_pP->vt_s,rlc_pP->vt_a) >= RLC_AM_DIFF_SN(ack_sn,rlc_pP->vt_a))
    if(rlc_am_in_tx_window(ctxt_pP, rlc_pP, ack_sn))
    {
//    	如果没有接收的状态PDU没有NACK参数，则只需要释放对端已成功接收的RLC PDU
      if (rlc_pP->control_pdu_info.num_nack == 0)
      {
    	  segmentation_pdu_mbP = list2_get_head(&rlc_pP->segmentation_pdu_list);

    	  do
    	  {
//    		  segmentation_pdu_mbP = list2_get_head(rlc_pP->segmentation_pdu_list);
    		  sn_cursor = ((rlc_am_tx_sdu_management_t *)(segmentation_pdu_mbP->data))->sn;

    		  if(sn_cursor != ack_sn)
    		  {
    			  list2_remove_head(&rlc_pP->segmentation_pdu_list);
    			  free_mem_block(segmentation_pdu_mbP, __func__);
    			  segmentation_pdu_mbP = list2_get_head(&rlc_pP->segmentation_pdu_list);
    		  }
    		  else
    		  {
    	  ...
    	  } while(segmentation_pdu_mbP != NULL);
...
//        sn_data_cnf = RLC_AM_PREV_SN(vt_a_new);
        sn_data_cnf = (vt_a_new - 1) % rlc_pP->tx_sn_modulo;
      }
      else // TODO 接收的状态PDU有NACK部分数据
      {
        nack_index = 0;
        nack_sn   = rlc_pP->control_pdu_info.nack_list[nack_index].nack_sn;
//        nack_sn = (rlc_pP->tx_sn_length == 18) ? rlc_pP->control_pdu_info.nack_list[nack_index].nack_u.nack18.nack_st.nack_sn : rlc_pP->control_pdu_info.nack_list[nack_index].nack_u.nack12.nack_st.nack_sn;
        prev_nack_sn = (rlc_pP->tx_sn_length == 18) ? 0x3FFFF: 0x0FFF;;

        segmentation_pdu_mbP = list2_get_head(&rlc_pP->segmentation_pdu_list);

//        第一个NACK SN之前的RLC SDU认为已经完整接收，释放发送缓冲
		  do
		  {
//    		  segmentation_pdu_mbP = list2_get_head(rlc_pP->segmentation_pdu_list);
			  sn_cursor = ((rlc_am_tx_sdu_management_t *)(segmentation_pdu_mbP->data))->sn;
...
//        }
    ...
    {
      printf(" WARNING CONTROL PDU ACK SN %d OUT OF WINDOW TX_Next_Ack=%d TX_Next=%d\n",
            ack_sn,rlc_pP->TX_Next_Ack,rlc_pP->TX_Next);
  ...
  {
    printf(" ERROR IN DECODING CONTROL PDU\n");
    *tb_size_in_bytes_pP = 0;
    status = FALSE;
  }
  ...
  if (status)
  {
      ...
      if ((rlc_pP->poll_sn != RLC_SN_UNDEFINED) &&
          (RLC_AM_DIFF_SN(ack_sn,rlc_pP->TX_Next_Ack) > RLC_AM_DIFF_SN(rlc_pP->poll_sn,rlc_pP->TX_Next_Ack)))
      {
...
//        	  rlc_am_pdu_sdu_data_cnf(ctxt_pP,rlc_pP,sn_cursor); //TODO need fixed
        	  sn_cursor = RLC_AM_NEXT_SN(sn_cursor);
          }

          // Handle last SN. TO DO : case of PDU partially ACKED with SDU to be data conf
          if (data_cnf_so_stop == 0x7FFF) {
//        	  rlc_am_pdu_sdu_data_cnf(ctxt_pP,rlc_pP,sn_data_cnf); //TODO need fixed
          }

          printf(" RECEIVE STATUS PDU ACK_SN=%d NewvtA=%d OldvtA=%d SnDataCnf=%d DataCnfSOStop=%d vtS=%d\n",
                      ack_sn,vt_a_new,rlc_pP->TX_Next_Ack,sn_data_cnf,data_cnf_so_stop,rlc_pP->TX_Next);
      }
      else {
          printf(" RECEIVE STATUS PDU WITH NO SDU CNF ACK_SN=%d NewvtA=%d OldvtA=%d vtS=%d\n",
                      ack_sn,vt_a_new,rlc_pP->TX_Next_Ack,rlc_pP->TX_Next);
      }
...
int rlc_am_write_status_pdu(
	const protocol_ctxt_t* const     ctxt_pP,
	rlc_am_entity_t *const           rlc_pP,
	rlc_am_control_ack_t* const        ack_pP,
	rlc_am_control_pdu_info_t* const pdu_info_pP)
{
	uint8_t* byte_pos_p = ((uint8_t*)ack_pP) + 3;
	...
	if(rlc_pP->rx_sn_length == 18) {
		ack_pP->ack18.ack_sn = pdu_info_pP->ack_sn;
	} else {
		ack_pP->ack12.ack_sn = pdu_info_pP->ack_sn;
	}
	...
	for(index = 0; index < pdu_info_pP->num_nack; index++) {
		if(rlc_pP->rx_sn_length == 18) {
			((rlc_am_control_nack18_t*)byte_pos_p)->nack_st.nack_sn = pdu_info_pP->nack_list[index].nack_sn;
			((rlc_am_control_nack18_t*)byte_pos_p)->nack_st.e1 = pdu_info_pP->nack_list[index].e1;
			((rlc_am_control_nack18_t*)byte_pos_p)->nack_st.e2 = pdu_info_pP->nack_list[index].e2;
			((rlc_am_control_nack18_t*)byte_pos_p)->nack_st.e3 = pdu_info_pP->nack_list[index].e3;
			byte_pos_p += 3;
			num_bytes += 3;
		} else {
			((rlc_am_control_nack12_t*)byte_pos_p)->nack_st.nack_sn = pdu_info_pP->nack_list[index].nack_sn;
			((rlc_am_control_nack12_t*)byte_pos_p)->nack_st.e1 = pdu_info_pP->nack_list[index].e1;
			((rlc_am_control_nack12_t*)byte_pos_p)->nack_st.e2 = pdu_info_pP->nack_list[index].e2;
			((rlc_am_control_nack12_t*)byte_pos_p)->nack_st.e3 = pdu_info_pP->nack_list[index].e3;
			byte_pos_p += 2;
			num_bytes += 2;
		}

		if(pdu_info_pP->nack_list[index].e2 == 1) {
			*((uint16_t*)byte_pos_p) = pdu_info_pP->nack_list[index].so_start;
			byte_pos_p += 2;
			*((uint16_t*)byte_pos_p) = pdu_info_pP->nack_list[index].so_end;
			byte_pos_p += 2;

			num_bytes += 4;
		}

		if(pdu_info_pP->nack_list[index].e3 == 1) {
			*byte_pos_p = pdu_info_pP->nack_list[index].nack_range;
			byte_pos_p += 1;
...
//-----------------------------------------------------------------------------
#if 1
void rlc_am_send_status_pdu(const protocol_ctxt_t* const  ctxt_pP, rlc_am_entity_t *const rlc_pP)
{
//	3GPP 38322 f60 5.3.4
...
//		-	set the ACK_SN to the SN of the next not received RLC SDU which is not indicated as missing in the resulting STATUS PDU.
	...
	signed int nb_byte_to_transmit = rlc_pP->nb_bytes_requested_by_mac;
...
//	rlc_sn_t                      sn_nack               = rlc_pP->vr_r;
	...
	if (rlc_pP->RX_Next == rlc_pP->RX_Highest_Status) {

		control_pdu_info.ack_sn = rlc_pP->RX_Highest_Status;
		status_report_completed = TRUE;

		printf("[SEND-STATUS] LINE %d ALL ACK WITH ACK_SN %04d\n", __LINE__, rlc_pP->RX_Highest_Status);
	}
	else if ((cursor_p != NULL) && ((nb_byte_transmitted + sizeof(rlc_am_control_nack18_t)) <= nb_bits_to_transmit)) // TODO 此部分长度算法还存在问题
	{
		//	  当前节点不为空，且剩余数据空间满足nack长度需求
		while(cursor_p != NULL && (nb_byte_to_transmit >= nb_byte_transmitted + sizeof(rlc_am_control_nack18_t))) {
			pdu_info_cursor_p       = &((rlc_am_rx_pdu_management_t*)(cursor_p->data))->pdu_info; // 当前接收数据缓冲信息
			if(pdu_info_previous_p != NULL) {
				pdu_info_previous_p       = &((rlc_am_rx_pdu_management_t*)(previous_p->data))->pdu_info;
			}

			if(waited_sn == pdu_info_cursor_p->sn) // 同一个RLC PDU已接收数据块
			{
				if(waited_so >= pdu_info_cursor_p->so) // 接收数据块连续
				{
					waited_so = pdu_info_cursor_p->so + pdu_info_cursor_p->payload_size;

					if(pdu_info_cursor_p->header.header_st.si == RLC_SI_DATA_FIELD_CONTAINS_THE_LAST_SEGMENT_OF_AN_RLC_SDU) {
						waited_so = 0;
						waited_sn = (pdu_info_cursor_p->sn + 1) % rlc_pP->rx_sn_modulo;
					}
				...
				{
					if(control_pdu_info.nack_list[control_pdu_info.num_nack].e2 == 1) // 如果当前RLC PDU已存在一个丢失分段
					{
						control_pdu_info.nack_list[control_pdu_info.num_nack].e1 = 1;
						control_pdu_info.num_nack += 1;
					}

					control_pdu_info.nack_list[control_pdu_info.num_nack].nack_sn = pdu_info_cursor_p->sn;
					...
					nb_byte_transmitted += ((rlc_pP->rx_sn_length == 18) ? 3 : 2) + 4;
			...
			{
				if(pdu_info_previous_p->header.header_st.si != RLC_SI_DATA_FIELD_CONTAINS_THE_LAST_SEGMENT_OF_AN_RLC_SDU) // 前一个RLC PDU缺少末尾数据段
				{
					control_pdu_info.nack_list[control_pdu_info.num_nack].e1 = 1;
					control_pdu_info.num_nack += 1;

					control_pdu_info.nack_list[control_pdu_info.num_nack].nack_sn = pdu_info_previous_p->sn;
					control_pdu_info.nack_list[control_pdu_info.num_nack].e2 = 1;
					control_pdu_info.nack_list[control_pdu_info.num_nack].so_start = pdu_info_previous_p->so + pdu_info_previous_p->payload_size;
					control_pdu_info.nack_list[control_pdu_info.num_nack].so_end = 0xFFFF;

					nb_byte_transmitted += ((rlc_pP->rx_sn_length == 18) ? 3 : 2) + 4;

					if(pdu_info_cursor_p->sn != (pdu_info_previous_p->sn + 1) % rlc_pP->rx_sn_modulo) // 存在完全丢失的RLC PDU，则当前NACK存在RANGE字段
					{
						control_pdu_info.nack_list[control_pdu_info.num_nack].e3 = 1;
						control_pdu_info.nack_list[control_pdu_info.num_nack].nack_range = (pdu_info_cursor_p->sn + rlc_pP->rx_sn_modulo - pdu_info_previous_p->sn - 1) % rlc_pP->rx_sn_modulo;

						nb_byte_transmitted += 1;
					}

					waited_sn = pdu_info_cursor_p->sn;
					waited_so = pdu_info_cursor_p->so + pdu_info_cursor_p->payload_size;

					previous_p = cursor_p;
					cursor_p = cursor_p->next;
				}
				else // 前一个RLC PDU的末尾数据段已经接收
				{
					if(pdu_info_cursor_p->sn == (pdu_info_previous_p->sn + 2) % rlc_pP->rx_sn_modulo) // 只丢失一个完整的RLC PDU
					{
						control_pdu_info.nack_list[control_pdu_info.num_nack].e1 = 1;
						control_pdu_info.num_nack += 1;

						control_pdu_info.nack_list[control_pdu_info.num_nack].nack_sn = (pdu_info_previous_p->sn + 1) % rlc_pP->rx_sn_modulo;
						control_pdu_info.nack_list[control_pdu_info.num_nack].e2 = 1;
					...
					if(pdu_info_cursor_p->so != 0) // 当前RLC PDU起始数据未接收
					{
						control_pdu_info.nack_list[control_pdu_info.num_nack].e1 = 1;
						control_pdu_info.num_nack += 1;
		...
		cursor_p = list2_get_tail(&rlc_pP->receiver_buffer);
		pdu_info_cursor_p = &((rlc_am_rx_pdu_management_t*)(cursor_p->data))->pdu_info;

		control_pdu_info.ack_sn = (pdu_info_cursor_p->sn + 1) % rlc_pP->rx_sn_modulo;
	} else {
	/* reception buffer empty or not enough TBS for filling at least 1 NACK_SN + E1 + E2 */
		control_pdu_info.ack_sn = rlc_pP->RX_Next;
#if TRACE_RLC_AM_STATUS_CREATION
...
//	}
	...
	pdu_size);
	#endif
	tb_p = get_free_mem_block(sizeof(struct mac_tb_req) + nb_byte_transmitted, __func__);
...
//	需要增加MAC subheader长度
	if(pdu_size <= 0xFF) {
		rlc_pP->nb_bytes_requested_by_mac = rlc_pP->nb_bytes_requested_by_mac - pdu_size - 2;
	} else {
		rlc_pP->nb_bytes_requested_by_mac = rlc_pP->nb_bytes_requested_by_mac - pdu_size - 3;
	}
//	put pdu in trans
...
//	-	set the ACK_SN to the SN of the next not received RLC SDU which is not indicated as missing in the resulting STATUS PDU.
  ...
  signed int                    nb_bits_to_transmit   = rlc_pP->nb_bytes_requested_by_mac << 3;
  int32_t nb_bytes_requested = rlc_pP->nb_bytes_requested_by_mac;
  int32_t status_pdu_l = nb_bytes_requested - 3;
  // minimum header size in bits to be transmitted: D/C + CPT + ACK_SN + E1
  signed int                    nb_bits_transmitted	  = RLC_AM_PDU_D_C_BITS + RLC_AM_STATUS_PDU_CPT_LENGTH + RLC_AM_SN_BITS + RLC_AM_PDU_E_BITS;
  rlc_am_control_pdu_info_t     control_pdu_info;
  rlc_am_pdu_info_t            *pdu_info_cursor_p     = NULL;
...
//  rlc_sn_t                      sn_nack               = rlc_pP->vr_r;
  int32_t                      sn_cursor             = 0;
  int32_t                      sn_nack               = rlc_pP->RX_Highest_Status;
  int32_t							start_nack_sn				= rlc_pP->RX_Next;
  mem_block_t                  *cursor_p              = rlc_pP->receiver_buffer.head;
  int                           all_segments_received = 0;
  int                           waited_so             = 0;
  mem_block_t                  *tb_p                  = NULL;
  ...
  boolean_t						segment_loop_end	  = FALSE;
  rlc_am_control_nack_t nack;
  rlc_am_rx_pdu_management_t* pdu_mng_cursor_pP = NULL;
  rlc_am_rx_pdu_management_t* pdu_mng_previous_pP = NULL;
  uint16_t so_start = 0;
  uint16_t so_end = 0xFFFF;
  int32_t num_nack = 0;
...
//                PROTOCOL_RLC_AM_CTXT_ARGS(ctxt_pP,rlc_pP),
                __LINE__,
				rlc_pP->RX_Highest_Status);
#endif
  }
  else if ((cursor_p != NULL) && (status_pdu_l + 3 <= nb_bytes_requested))
  {
//	  ACK字段长度
	  pdu_size += 3;
	  nb_bytes_requested -= 3;

	  nack.nack_sn = ((rlc_am_rx_pdu_management_t*)(cursor_p->data))->pdu_info.sn;

	  if(((rlc_am_rx_pdu_management_t*)(cursor_p->data))->pdu_info.so != nack.so_end)
	  {
		  nack.so_end = ((rlc_am_rx_pdu_management_t*)(cursor_p->data))->pdu_info.so;
	  }

//	  填充NACK字段
	  do
	  {
		  pdu_mng_cursor_pP = (rlc_am_rx_pdu_management_t*)(cursor_p->data);
		  pdu_info_cursor_p = pdu_mng_cursor_pP->pdu_info;

//		  第一个接收缓冲节点
		  if(cursor_p->previous == NULL)
		  {
			  nack.nack_sn = ((rlc_am_rx_pdu_management_t*)(cursor_p->data))->pdu_info.sn;

			  if(((rlc_am_rx_pdu_management_t*)(cursor_p->data))->pdu_info.so != 0)
			  {
				  nack.so_end = ((rlc_am_rx_pdu_management_t*)(cursor_p->data))->pdu_info.so;
			  }
		  }
		  ...
		  {
			  if(pdu_mng_pP->all_segments_received == 0)
			  {
//				  当前节点SN与NACK SN相同
				  if(pdu_info_cursor_p->sn == nack.nack_sn)
				  {
//					  分段连续
					  if(pdu_info_cursor_p->so == nack.so_end)
					  {
						  if(pdu_info_cursor_p->header.header_st.si == RLC_SI_DATA_FIELD_CONTAINS_THE_LAST_SEGMENT_OF_AN_RLC_SDU)
						  {
						  }
					  ...
					  {
						  nack.so_end = pdu_info_cursor_p->so; // 赋值NACK字段的SO_END
						  nack.e2 = 1; // SO字段存在标志
						  memcpy(&control_pdu_info.nack_list[num_nack], &nack, sizeof(rlc_am_control_nack_t));

						  if(pdu_info_cursor_p->header.header_st.si != RLC_SI_DATA_FIELD_CONTAINS_THE_LAST_SEGMENT_OF_AN_RLC_SDU)
						  {
							  nack.so_start = pdu_info_cursor_p->so;
							  nack.e2 = 0;
							  num_nack += 1;
						  }
					  }
...
//		  pdu_size,rlc_pP->nb_bytes_requested_by_mac,nb_bits_transmitted, rlc_pP->channel_id);
	if(pdu_size > rlc_pP->nb_bytes_requested_by_mac)
	{
		printf("RLC AM Tx Status PDU Data size=%d bigger than remaining TBS=%d nb_bits_transmitted=%d LcId=%d\n",
		  pdu_size,rlc_pP->nb_bytes_requested_by_mac,nb_bits_transmitted, rlc_pP->channel_id);
		return;
	}
