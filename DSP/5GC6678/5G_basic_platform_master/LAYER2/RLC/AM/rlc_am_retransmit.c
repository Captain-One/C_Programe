  ...
  sdu_size_t so_endP)
{
  ...
  if (mb_p != NULL) {
    ...
    if(so_startP > so_endP) {
      printf("[NACK-PDU] ERROR NACK MISSING PDU, so_startP %d, so_endP %d\n", so_startP, so_endP);
      status = FALSE;
    }
    // Handle full PDU NACK first
    	...
    	if ((prev_nack_snP != snP) && (tx_data_pdu_buffer_p->flags.ack == 0) && (tx_data_pdu_buffer_p->flags.max_retransmit == 0)) {
            ...
            tx_data_pdu_buffer_p->nack_so_start[so_startP] = 0;
            tx_data_pdu_buffer_p->num_holes     = 0;
            tx_data_pdu_buffer_p->retx_hole_index = 0;
            tx_data_pdu_buffer_p->nack_so_stop[so_endP]  = tx_data_pdu_buffer_p->payload_size - 1;
        #if TRACE_RLC_AM_HOLE
            printf("[HOLE] SN %04d GLOBAL NACK 0->%05d\n",
            ...
            //assert(tx_data_pdu_buffer_p->nack_so_start < tx_data_pdu_buffer_p->payload_size);
              ...
              printf("[NACK-PDU] ERROR NACK MISSING PDU, nack_so_start %d, payload_size %d\n",
                    tx_data_pdu_buffer_p->nack_so_start, tx_data_pdu_buffer_p->payload_size);
    	        status = FALSE;
    	      }
    	}
    	else {
		...
		if ((so_startP <= so_endP) && (so_endP < tx_data_pdu_buffer_p->payload_size)) {
	    	if (prev_nack_snP != snP) {
	            ...
	            tx_data_pdu_buffer_p->nack_so_start[so_startP] = so_startP;
	            tx_data_pdu_buffer_p->nack_so_stop[so_endP] = so_endP;
    ...
    printf("[NACK-PDU] ERROR NACK MISSING PDU SN %05d\n", snP);
    status = FALSE;
  }
  ...
  boolean_t free_pdu)
{
  ...
  if (mb_p != NULL) {
    ...
    printf("[ACK-PDU] ACK PDU SN %05d previous retx_count %d \n", snP, tx_data_pdu_buffer->retx_count);
    }
...
//Assertion(eNB)_PRAN_DesignDocument_annex No.768
      ...
      {
        printf("RLC AM Rx Status Report sn=%d acked twice but is pending for Retx vtA=%d vtS=%d LcId=%d\n",
              snP, rlc_pP->TX_Next_Ack,rlc_pP->TX_Next,rlc_pP->channel_id);
         return;
      }
/*
...
*/
    ...
    printf("[ACK-PDU] WARNING ACK PDU SN %05d -> NO PDU TO ACK\n", snP);
...
*/
	...
	{
		printf("[RE-SEGMENT] OUT OF MEMORY PDU SN %04d\n",
		              PROTOCOL_RLC_AM_CTXT_ARGS(ctxt_pP,rlc_pP),
					  sn);
	}
...
{
  int       sn = rlc_pP->TX_Next_Ack;
  int            i, loop = 0;
  rlc_am_tx_sdu_management_t *tx_data_sdu_mng_p;

  if (message_pP)
  {
    printf(" Retransmission buffer %s TX_Next_Ack=0x%08x TX_Next=0x%08x:",
//          PROTOCOL_RLC_AM_CTXT_ARGS(ctxt_pP,rlc_pP),
          message_pP,
          rlc_pP->TX_Next_Ack,
          rlc_pP->TX_Next);
  ...
  {
    printf(" Retransmission buffer TX_Next_Ack=%04d TX_Next=%04d:",
//          PROTOCOL_RLC_AM_CTXT_ARGS(ctxt_pP,rlc_pP),
          rlc_pP->TX_Next_Ack,
          rlc_pP->TX_Next);
  }

  while (rlc_pP->TX_Next != sn)
  {
  ...
  char* const message_pP)
{
  ...
  if (message_pP) {
    printf(" Retransmission buffer %s VT(A)=%04d VT(S)=%04d:",
          PROTOCOL_RLC_AM_CTXT_ARGS(ctxt_pP,rlc_pP),
          message_pP,
          rlc_pP->vt_a,
          rlc_pP->vt_s);
  } else {
    printf(" Retransmission buffer VT(A)=%04d VT(S)=%04d:",
          PROTOCOL_RLC_AM_CTXT_ARGS(ctxt_pP,rlc_pP),
          rlc_pP->vt_a,
          rlc_pP->vt_s);
  }
...
#if 1
//-----------------------------------------------------------------------------
mem_block_t * rlc_am_get_pdu_to_retransmit(const protocol_ctxt_t* const ctxt_pP, rlc_am_entity_t* const rlc_pP)
{
	int32_t             sn          = rlc_pP->TX_Next_Ack;
	int32_t             sn_end      = rlc_pP->TX_Next;
	mem_block_t*         pdu_p        = NULL;
	mem_block_t*		tx_buf_p;
	rlc_am_tx_data_pdu_management_t* tx_data_pdu_management;
	rlc_am_tx_sdu_management_t* tx_sdu_management;
	rlc_am_control_pdu_info_t* control_pdu_info_pP = &rlc_pP->control_pdu_info;
	rlc_am_control_nack_t* nack_pP;

	int num_nack;
	uint16_t retx_size = 0;
	uint16_t headerL = rlc_pP->tx_sn_length == 18 ? 3 : 2;
	uint16_t retx_pdu_size = 0;
	uint16_t retx_start = 0;
	uint16_t retx_end = 0;
	mem_block_t* retx_mbP = NULL;
	rlc_am_header_t header_pdu;
//Assertion(eNB)_PRAN_DesignDocument_annex No.769
	if((rlc_pP->retrans_num_pdus <= 0) || (rlc_pP->TX_Next ==  rlc_pP->TX_Next_Ack)) {
		printf("RLC AM ReTx start process Error: NbPDUtoRetx=%d TX_Next=%d TX_Next_Ack=%d  LcId=%d !\n",
				rlc_pP->retrans_num_pdus,rlc_pP->TX_Next, rlc_pP->TX_Next_Ack, rlc_pP->channel_id);
		return NULL;
	}
/*
...
*/
	...
	tx_buf_p = &rlc_pP->tx_sdus.head;

	for(num_nack = 0; num_nack <= control_pdu_info_pP->num_nack; num_nack++) {
		nack_pP = &(control_pdu_info_pP->nack_list[num_nack]);

//		如果当前NACK已经重传，则获取下一个NACK数据
		if(nack_pP->retx) {
			continue;
		}else {
			tx_sdu_management = (rlc_am_tx_sdu_management_t*)(tx_buf_p->data);

			if(tx_sdu_management->retx_count > rlc_pP->max_retx_threshold) {
				printf("[RLC AM MAX RETX=%d] SN %04d\n", tx_sdu_management->retx_count, tx_sdu_management->sn);
			}

//			获取NACK_SN对应的SDU的缓冲
			do {
//				如果缓冲数据SN与NACK SN不同，则获取下一个缓冲块
				if(tx_sdu_management->sn != nack_pP->nack_sn) {
					tx_buf_p = tx_buf_p->next;
				} else {
					if(rlc_pP->tx_sn_length == 18) {
						header_pdu.h18.snh2 = (tx_sdu_management->sn >> 16) & 0x03;
						header_pdu.h18.snm = (tx_sdu_management->sn >> 8) & 0xFF;
						header_pdu.h18.snl = tx_sdu_management->sn & 0xFF;
					} else {
						header_pdu.h12.snh4 = (tx_sdu_management->sn >> 8) & 0x0F;
						header_pdu.h12.snl = tx_sdu_management->sn & 0xFF;
					}

					if(nack_pP->e2) {// 存在丢失分段
						headerL += 2; // 数据头需要包含SO字段，2字节长度

//						获取需要重传的分段数据长度
						if(nack_pP->so_end <= tx_sdu_management->sdu_size) { //不是最后一个分段
							retx_size = nack_pP->so_end - nack_pP->so_start + 1; // 获取丢失分段长度
						} else {
							retx_size = tx_sdu_management->sdu_size - nack_pP->so_start + 1;
						}

//						判断数据空间是否足够，如果不够，则计算可以重传数据大小
						if((retx_size + headerL) > rlc_pP->nb_bytes_requested_by_mac) {
							retx_size = rlc_pP->nb_bytes_requested_by_mac - headerL;

//							更新本NACK分段起始偏移地址，保存重传分段起始地址
							retx_start = nack_pP->so_start;
							nack_pP->so_start += retx_size;
						}
						else {// 如果数据空间足够，则设置本NACK分段重传完毕
							if(nack_pP->e3 == 0) {// 如果不存在NACK_RANGE字段，则本NACK部分传输完成
								nack_pP->retx = 1;
							} else {
								nack_pP->nack_range -= 1;
								nack_pP->nack_sn = (nack_pP->nack_sn + 1) % rlc_pP->tx_sn_modulo;

								if(nack_pP->nack_range == 0) {
									nack_pP->e3 = 0;
								}
							}
						}
						...
						retx_start = 0;
						retx_size = tx_sdu_management->sdu_size; // 整个PDU未接收

//						如果分配的上行空间小于重传需要的数据空间，则需要对SDU数据分段
						if((retx_size + headerL) > rlc_pP->nb_bytes_requested_by_mac) {
							headerL += 2;
							...
							nack_pP->e2 = 1;
							nack_pP->so_start = retx_size;
							nack_pP->so_end = tx_sdu_management->sdu_size - 1;
						} else {
							if(nack_pP->e3 == 0) {
								nack_pP->retx = 1;
							} else {//如果存在连续的SDU丢失，并且当前SDU重传完成，则NACK_SN+1,连续NACK_RANGE-1，如果剩余连续范围=0,则设置本分段连续丢失标识为0
								nack_pP->nack_sn = (nack_pP->nack_sn + 1) % rlc_pP->tx_sn_modulo;
								nack_pP->nack_range -= 1;
					...
					//					  产生重传RLC PDU
					retx_mbP = get_free_mem_block(retx_size + headerL, __func__);

					if(retx_mbP == NULL) {
						printf("Can't get mem_block for retransmit!\n");
						return NULL;
					}

					memset(retx_mbP->data, 0, retx_size + headerL);

					if(retx_start == 0) {
						if(retx_size == tx_sdu_management->sdu_size) {
							if(rlc_pP->tx_sn_length == 18) {
								header_pdu.h18.si = RLC_SI_DATA_FIELD_CONTAINS_ALL_BYTES_OF_AN_RLC_SDU;
							} else {
								header_pdu.h12.si = RLC_SI_DATA_FIELD_CONTAINS_ALL_BYTES_OF_AN_RLC_SDU;
							}
						} else {
							if(rlc_pP->tx_sn_length == 18) {
								header_pdu.h18.si = RLC_SI_DATA_FIELD_CONTAINS_THE_FIRST_SEGMENT_OF_AN_RLC_SDU;
							} else {
								header_pdu.h12.si = RLC_SI_DATA_FIELD_CONTAINS_THE_FIRST_SEGMENT_OF_AN_RLC_SDU;
							}
						}
					} else {
						if(retx_start + retx_size == tx_sdu_management->sdu_size) {
							if(rlc_pP->tx_sn_length == 18) {
								header_pdu.h18.si = RLC_SI_DATA_FIELD_CONTAINS_THE_LAST_SEGMENT_OF_AN_RLC_SDU;
							} else {
								header_pdu.h12.si = RLC_SI_DATA_FIELD_CONTAINS_THE_LAST_SEGMENT_OF_AN_RLC_SDU;
							}
						} else {
							if(rlc_pP->tx_sn_length == 18) {
								header_pdu.h18.si = RLC_SI_DATA_FIELD_CONTAINS_NEITHER_THE_FIRST_NOR_LAST_SEGMENT_OF_AN_RLC_SDU;
							} else {
								header_pdu.h12.si = RLC_SI_DATA_FIELD_CONTAINS_NEITHER_THE_FIRST_NOR_LAST_SEGMENT_OF_AN_RLC_SDU;
							}
						}
					}
					...
					if(retx_size != tx_sdu_management->sdu_size) {
						if(rlc_pP->tx_sn_length == 18) {
							header_pdu.h18.soh = (retx_start >> 8) & 0xFF;
							header_pdu.h18.sol = retx_start & 0xFF;
						} else {
							header_pdu.h12.soh = (retx_start >> 8) & 0xFF;
							header_pdu.h12.sol = retx_start & 0xFF;
						}
					}

					memcpy(retx_mbP->data + headerL, tx_sdu_management->first_byte + retx_start, retx_size);

//					增加 MAC subheader before MAC SDU（RLC PDU）
					if(retx_size <= 0xFF) {
						rlc_pP->nb_bytes_requested_by_mac -= retx_size + 2;
					} else {
						rlc_pP->nb_bytes_requested_by_mac -= retx_size + 3;
					}
					...
					rlc_pP->stat_tx_data_bytes                 += retx_size;
					rlc_pP->stat_tx_retransmit_bytes           += retx_size;
					rlc_pP->stat_tx_retransmit_bytes_by_status += retx_size;

					tx_sdu_management->retx_count += 1;
