...
//Assertion(eNB)_PRAN_DesignDocument_annex No.788
      if(cursor_p == NULL)
      {
         printf("AM Rx PDU Error, received buffer empty LcID=%d\n",rlc_pP->channel_id);
         return RLC_AM_DATA_PDU_STATUS_HEADER_ERROR;
      }
/*
	  AssertFatal(cursor_p != NULL,"AM Rx PDU Error, received buffer empty LcID=%d\n",rlc_pP->channel_id);
*/
	  do
	  {
		  ...
		  // Stop if Cursor SN >= Received SN
		  if(((pdu_info_cursor_p->sn - rlc_pP->RX_Next) % rlc_pP->rx_sn_modulo) >=
				  ((pdu_rx_info_p->sn - rlc_pP->RX_Next) % rlc_pP->rx_sn_modulo))
		  {
			  break;
		  }
	  ...
	  while (cursor_p != NULL); // end of list
	  ...
	  if ((cursor_p == NULL) || (pdu_info_cursor_p->sn != pdu_rx_info_p->sn))
	  {
	      if (previous_cursor_p != NULL)
	      {
			   printf("[PROCESS RX PDU SN=%d] PDU INSERTED AFTER PDU SN=%d\n",
					pdu_rx_info_p->sn,
	      ...
	      { /* SN of head of Rx PDU list is higher than received PDU SN */
              printf("[PROCESS RX PDU SN=%d] PDU INSERTED BEFORE PDU SN=%d\n",
					pdu_rx_info_p->sn,
					pdu_info_cursor_p->sn);
	          list2_insert_before_element(tb_pP, cursor_p, &rlc_pP->receiver_buffer);
	      }
...
//	  从第一个与接收数据SN相同的节点开始处理数据重复
//	  接收分段偏移地址不会位于已接收分段中
	  while ((cursor_p != NULL) && (pdu_info_cursor_p->sn == pdu_rx_info_p->sn))
	  {
		  if(pdu_info_cursor_p->so == pdu_rx_info_p->so) // 偏移地址相同
		  {
			  if(pdu_info_cursor_p->payload_size >= pdu_rx_info_p->payload_size) // 已保存数据包含接收数据
			  {
				  return RLC_AM_DATA_PDU_STATUS_AM_SEGMENT_DUPLICATE;
			  }
			  else // 接收数据大于已存储数据，则从接收数据中删去已存储数据
			  {
				  pdu_rx_info_p->so += pdu_info_cursor_p->payload_size;
				  pdu_rx_info_p->payload_size -= pdu_info_cursor_p->payload_size;
				  pdu_rx_info_p->payload += pdu_info_cursor_p->payload_size;

				  if(pdu_rx_info_p->payload_size <= 0)
				  {
					  return RLC_AM_DATA_PDU_STATUS_AM_SEGMENT_DUPLICATE;
				  }
			  }
		  ...
		  {
			  if(pdu_info_cursor_p->so > pdu_rx_info_p->so)
			  {
//				  接收分段数据不超过当前节点数据域
				  if(pdu_rx_info_p->so + pdu_rx_info_p->payload_size <= pdu_info_cursor_p->so + pdu_info_cursor_p->payload_size)
				  {
//					  当接收数据包含部分节点数据时，接收数据截断超过截断偏移地址的数据
					  if(pdu_rx_info_p->so + pdu_rx_info_p->payload_size >= pdu_info_cursor_p->so)
					  {
						  pdu_rx_info_p->payload_size = pdu_info_cursor_p->so - pdu_rx_info_p->so;
					  }
					  break;
				  }
				  else // 接收数据包含当前节点数据，移除当前节点数据，继续下一个节点数据处理
				  {
					  list2_remove_element(cursor_p, &rlc_pP->receiver_buffer);
					  free_mem_block(cursor_p, __func__);
	  ...
	  if((pdu_info_cursor_p->sn != pdu_rx_info_p->sn) || (cursor_p == NULL))
	  {
		  list2_insert_after_element(tb_pP, previous_cursor_p, &rlc_pP->receiver_buffer);
	  }
	  else
  ...
  mem_block_t* const tb_pP)
{
	  ...
	  if (cursor_p == NULL)
	  {
		    printf("[INSERT PDU] LINE %d RX PDU SN %04d (only inserted)\n",
		          __LINE__,
				  pdu_rx_info_p->sn);
  ...
  mem_block_t* const tb_pP)
{
  ...
  if (pdu_info_p->header.header_st.si == RLC_SI_DATA_FIELD_CONTAINS_ALL_BYTES_OF_AN_RLC_SDU)
  {
  ...
  while (cursor_p->next != NULL)
  {
    ...
    if (pdu_info_p->sn == sn)
    {
        ...
        if (pdu_info_p->header.header_st.si == RLC_SI_DATA_FIELD_CONTAINS_THE_LAST_SEGMENT_OF_AN_RLC_SDU)
        {
...
#if 0
//  找到最后一个分段，计算RLC SDU总长度
  while((pdu_info_p->header.header_st.si != RLC_SI_DATA_FIELD_CONTAINS_ALL_BYTES_OF_AN_RLC_SDU) &&
		  (pdu_info_p->header.header_st.si != RLC_SI_DATA_FIELD_CONTAINS_THE_LAST_SEGMENT_OF_AN_RLC_SDU) && (sn == pdu_info_p->sn))
  {
	  cursor_p = cursor_p->next;
  ...
  list2_t * list_pP)
{
  ...
  {
    //assert(2==1);
    printf("list2_insert_before_element error. element_to_insert_pP %p, element_pP %p\n", element_to_insert_pP,element_pP);
    return NULL;
  }
}
//-----------------------------------------------------------------------------
  ...
  list2_t * list_pP)
{
  ...
  {
    //assert(2==1);
    printf("list2_insert_after_element error. element_to_insert_pP %p, element_pP %p\n", element_to_insert_pP,element_pP);
    return NULL;
  }
}
//-----------------------------------------------------------------------------
