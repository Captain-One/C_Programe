...
//Assertion(eNB)_PRAN_DesignDocument_annex No.766
  if(pdu_info_pP->header.header_st.d_c == 0)
  {
     printf("RLC AM Rx PDU Data D/C Header Error LcId=%d\n", rlc_pP->channel_id);
     return -2;
  }
/*
...
*/
    ...
    {
    	pdu_info_pP->sn  = *(header_pP + 2) + ((uint32_t)(*(header_pP + 1)) << 8) + (((uint32_t)(*header_pP & 0x03)) << 16);

    	if(pdu_info_pP->header.header_st.si)
    	{
    		pdu_info_pP->so = *(header_pP + 4) + ((uint16_t)(*(header_pP + 3)) << 8);
    		pdu_info_pP->header_size = 5;
    	}
    }
    else
    {
    	pdu_info_pP->sn  = *(header_pP + 1) + (((uint32_t)(*header_pP & 0x0F)) << 8);

    	if(pdu_info_pP->header.header_st.si)
		{
			pdu_info_pP->so = *(header_pP + 3) + ((uint16_t)(*(header_pP + 2)) << 8);
			pdu_info_pP->header_size = 4;
		}
    }
...
#endif
// assumed there is no missing byte segment of the SDU associated with SN = RX_Next before the last byte of all received segments of this SDU
//-----------------------------------------------------------------------------
boolean_t
rlc_am_rx_RX_Next_no_hole(
  const protocol_ctxt_t* const ctxt_pP,
  rlc_am_entity_t * const rlc_pP)
{
	boolean_t ret = FALSE;
...
//-----------------------------------------------------------------------------
void
rlc_am_rx_update_RX_Highest_Status(
  const protocol_ctxt_t* const ctxt_pP,
  rlc_am_entity_t * const rlc_pP,
  mem_block_t* tb_pP)
{
  ...
  if (cursor_p)
  {
    do
    {
      ...
      if ((((rlc_am_rx_pdu_management_t*)(cursor_p->data))->all_segments_received == 0) ||
          (rlc_pP->RX_Highest_Status != pdu_info_cursor_p->sn))
      {
      ...
      if(pdu_info_cursor_p->header.header_st.si == RLC_SI_DATA_FIELD_CONTAINS_ALL_BYTES_OF_AN_RLC_SDU ||
			  pdu_info_cursor_p->header.header_st.si == RLC_SI_DATA_FIELD_CONTAINS_THE_LAST_SEGMENT_OF_AN_RLC_SDU)
      {
    	  rlc_pP->RX_Highest_Status = (pdu_info_cursor_p->sn + 1) % rlc_pP->rx_sn_modulo;
      }
...
// assumed the sn of the tb_p is equal to RX_Next
//-----------------------------------------------------------------------------
void
rlc_am_rx_update_RX_Next(
  const protocol_ctxt_t* const ctxt_pP,
  rlc_am_entity_t * const rlc_pP,
  mem_block_t* tb_pP)
{
  //rlc_am_pdu_info_t* pdu_info_p        = &((rlc_am_rx_pdu_management_t*)(tb_pP->data))->pdu_info;
  rlc_am_pdu_info_t* pdu_info_cursor_p = NULL;
  rlc_am_rx_pdu_management_t* pdu_mgnt_cursor_p = NULL;
  mem_block_t*       cursor_p;
...
//  cursor_p = tb_pP;
  ...
  if (cursor_p)
  {
    do
    {
      pdu_mgnt_cursor_p = (rlc_am_rx_pdu_management_t *) (cursor_p->data);
      pdu_info_cursor_p = &pdu_mgnt_cursor_p->pdu_info;

      if ((pdu_mgnt_cursor_p->all_segments_received == 0) ||
          (rlc_pP->RX_Next != pdu_info_cursor_p->sn))
      {
        return;
      }
      ...
      if(pdu_info_cursor_p->header.header_st.si == RLC_SI_DATA_FIELD_CONTAINS_ALL_BYTES_OF_AN_RLC_SDU ||
    		  pdu_info_cursor_p->header.header_st.si == RLC_SI_DATA_FIELD_CONTAINS_THE_LAST_SEGMENT_OF_AN_RLC_SDU)
      {
    	  rlc_pP->RX_Next = (pdu_info_cursor_p->sn + 1) % rlc_pP->rx_sn_modulo;
      }

      previous_cursor_p = cursor_p;
      cursor_p = previous_cursor_p->next;

      if(pdu_mgnt_cursor_p->segment_reassembled == RLC_AM_RX_PDU_SEGMENT_REASSEMBLED)
      {
    	  list2_remove_element(previous_cursor_p, &rlc_pP->receiver_buffer);

    	  free_mem_block(previous_cursor_p, __func__);
      }
    }
    while (cursor_p != NULL);
...
//				Assertion(eNB)_PRAN_DesignDocument_annex No.767
				if(tb_size_in_bytes != 0)
				{
					printf("Remaining %d bytes following a control PDU\n", tb_size_in_bytes);
				}
				/*
			...
			printf("[RX ROUTING] VR(R)=%03d VR(MR)=%03d\n", rlc_pP->RX_Next, rlc_pP->RX_Next_Highest);
		}
	} // end while
...
{
	//	3GPP 38.322 5.2.3.2	Receive operations
	//	5.2.3.2.1	General
	...
	//解析数据，获取头部信息
	if(rlc_am_get_data_pdu_infos(ctxt_pP,rlc_pP, first_byte_pP, tb_size_in_bytesP, pdu_info_p) >= 0)
	{
		...
		if (rlc_am_in_rx_window(ctxt_pP, rlc_pP, pdu_info_p->sn))
		{
			...
			printf("[PROCESS RX PDU SN=%04d] RX_Next %08d RX_Next_Highest %08d RX_Highest_Status %08d RX_Next_Status_Trigger %08d rx_am_window_size %08d\n",
						pdu_info_p->sn,
						rlc_pP->RX_Next,
						rlc_pP->RX_Next_Highest,
						rlc_pP->RX_Highest_Status,
						rlc_pP->RX_Next_Status_Trigger,
						rlc_pP->rx_am_window_size);
			...
			if (pdu_status != RLC_AM_DATA_PDU_STATUS_OK)
			{
				...
				printf("[PROCESS RX PDU]  PDU DISCARDED CAUSE=%d SN=%d\n", pdu_status,pdu_info_p->sn);
			...
			else
			{
				...
				/* 1) Update RX_Next_Highest if sn >= RX_Next_Highest */
				if(rlc_am_modular_comparison(ctxt_pP, rlc_pP, 0, pdu_info_p->sn, rlc_pP->RX_Next_Highest))
				{
					rlc_pP->RX_Next_Highest = (pdu_info_p->sn + 1) % rlc_pP->rx_sn_modulo;
				}

				//        if(((pdu_info_p->sn - rlc_pP->RX_Next) % rlc_pP->rx_sn_modulo) >=
				//        		((rlc_pP->RX_Next_Highest - rlc_pP->RX_Next) % rlc_pP->rx_sn_modulo))
				//        {
				//        	rlc_pP->RX_Next_Highest = (pdu_info_p->sn + 1) % rlc_pP->rx_sn_modulo;
				//        }
				...
				{
					rlc_am_rx_list_reassemble_rlc_sdus(ctxt_pP, rlc_pP, tb_pP);

					if(pdu_info_p->sn == rlc_pP->RX_Highest_Status)
					{
						rlc_am_rx_update_RX_Highest_Status(ctxt_pP, rlc_pP, tb_pP);
					}

					if(pdu_info_p->sn == rlc_pP->RX_Next)
					{
						rlc_am_rx_update_RX_Next(ctxt_pP, rlc_pP, tb_pP);
					}
				}

				if(rlc_pP->t_Reassembly.running)
				{
					if(rlc_pP->RX_Next_Status_Trigger == rlc_pP->RX_Next)
					{
						rlc_pP->t_Reassembly.running = 0;
					}
					else if(rlc_pP->RX_Next_Status_Trigger == ((rlc_pP->RX_Next + 1) % rlc_pP->rx_sn_modulo))
					{
						if(rlc_am_rx_RX_Next_no_hole(ctxt_pP, rlc_pP))
						{
							rlc_pP->t_Reassembly.running = 0;
						}
					}
					else if(rlc_pP->RX_Next_Status_Trigger != ((rlc_pP->RX_Next + rlc_pP->rx_am_window_size) % rlc_pP->rx_sn_modulo))
					{
						if(rlc_am_in_rx_window(ctxt_pP, rlc_pP, rlc_pP->RX_Next_Status_Trigger) == 0)
						{
							rlc_pP->t_Reassembly.running = 0;
						}
					}
		...
		else
		{
			...
			printf("[PROCESS RX PDU]  PDU OUT OF RECEIVING WINDOW, DISCARDED, SN=%d\n", pdu_info_p->sn);
		}

		if ((pdu_info_p->header.header_st.p) && (pdu_status < RLC_AM_DATA_PDU_STATUS_BUFFER_FULL))
		{
			printf("[PROCESS RX PDU]  POLL BIT SET, STATUS REQUESTED:\n");
			...
			if ((RLC_AM_GET_STATUS(rlc_pP->status_requested,RLC_AM_STATUS_TRIGGERED_DELAYED)) ||
					!(RLC_AM_GET_STATUS(rlc_pP->status_requested,(RLC_AM_STATUS_TRIGGERED_POLL |
							RLC_AM_STATUS_TRIGGERED_T_REORDERING))))
			{
				...
				if ((pdu_status != RLC_AM_DATA_PDU_STATUS_OK) || ((pdu_status == RLC_AM_DATA_PDU_STATUS_OK) &&
					(!(RLC_AM_SN_IN_WINDOW(pdu_info_p->sn,rlc_pP->RX_Next)) ||
					(RLC_AM_DIFF_SN(pdu_info_p->sn,rlc_pP->RX_Next) < RLC_AM_DIFF_SN(rlc_pP->RX_Next_Status_Trigger,rlc_pP->RX_Next)))))
				{
