...
//-----------------------------------------------------------------------------
struct mac_data_ind mac_rlc_deserialize_tb(char *buffer_pP, const tb_size_t tb_sizeP, num_tb_t num_tbP, crc_t *crcs_pP)
{
//-----------------------------------------------------------------------------
	...
	while (num_tbP > 0) {
		...
		if(tb_p != NULL) {
			...
			printf("[MAC-RLC] DUMP RX PDU(%d bytes):\n", tb_sizeP);
...
//-----------------------------------------------------------------------------
tbs_size_t mac_rlc_serialize_tb(const logical_chan_id_t channel_idP, char* buffer_pP, list_t transport_blocksP)
{
//-----------------------------------------------------------------------------
	mem_block_t *tb_p;
	tbs_size_t   tbs_size;
	tbs_size_t   tb_size;
	NR_MAC_SUBHEADER_LONG mac_subh;

	tbs_size = 0;

	printf("list have %d elements!\n", transport_blocksP.nb_elements);

	while (transport_blocksP.nb_elements > 0) {
		memset(&mac_subh, 0, sizeof(NR_MAC_SUBHEADER_LONG));
		...
		if (tb_p != NULL) {
			...
			printf("rlc pdu has %d data!\n", tb_size);
#ifdef DEBUG_MAC_INTERFACE
			printf("[MAC-RLC] DUMP TX PDU(%d bytes):\n", tb_size);
			rlc_util_print_hex_octets(RLC, ((struct mac_tb_req *) (tb_p->data))->data_ptr, tb_size);
#endif

//			TODO 增加MAC subheader for MAC sdu(RLC PDU)
			mac_subh.LCID = channel_idP;
			if(tb_size <= 0xFF) {
				mac_subh.L1 = tb_size;

				memcpy(&buffer_pP[tbs_size], &mac_subh, 2);
				tbs_size += 2;
			} else {
				mac_subh.F = 1;
				mac_subh.L1 = tb_size >> 8;
				mac_subh.L2 = tb_size & 0x00FF;

				memcpy(&buffer_pP[tbs_size], &mac_subh, 3);
				tbs_size += 3;
			}
...
//-----------------------------------------------------------------------------
	...
	PROTOCOL_CTXT_SET_BY_MODULE_ID(&ctxt, module_idP, rntiP, frameP, subframeP, slotP);

#ifdef DEBUG_MAC_INTERFACE
	printf(PROTOCOL_CTXT_FMT" MAC_RLC_DATA_REQ channel %d (%d) MAX RB %d\n",
			PROTOCOL_CTXT_ARGS((&ctxt)),
			channel_idP, RLC_MAX_LC, NB_RB_MAX);
	...
	if(channel_idP >= NB_RB_MAX) {
		printf("channel id is too high (%u/%d)!\n", channel_idP, NB_RB_MAX);
		return 0;
	}

	key = RLC_COLL_KEY_LCID_VALUE(module_idP, rntiP, channel_idP, srb_flag);
	...
	if (h_rc == HASH_TABLE_OK) {
		...
		printf("RLC not configured lcid %u RNTI %x!\n", channel_idP, rntiP);
	}
	...
	case RLC_MODE_AM:
		...
		data_request = rlc_am_mac_data_request(&ctxt, &rlc_union_p->rlc.am, FALSE);
		ret_tb_size =mac_rlc_serialize_tb(channel_idP, buffer_pP, data_request.data);
		break;
	case RLC_MODE_UM:
		rlc_um_set_nb_bytes_requested_by_mac(&rlc_union_p->rlc.um, tb_sizeP);
		data_request = rlc_um_mac_data_request(&ctxt, &rlc_union_p->rlc.um, FALSE);
		ret_tb_size = mac_rlc_serialize_tb(channel_idP, buffer_pP, data_request.data);
		break;
	case RLC_MODE_TM:
