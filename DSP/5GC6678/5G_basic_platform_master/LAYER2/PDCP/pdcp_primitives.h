 ...
 * 6.2.2.1 Control Plane PDCP Data PDU
 */
#define PDCP_CONTROL_PLANE_DATA_PDU_SN_SIZE 2
#define PDCP_CONTROL_PLANE_DATA_PDU_MAC_I_SIZE 4
typedef struct {
	uint32_t sn;      // PDCP sequence number will wrap around 2^12-1 so
...
typedef struct {
	...
	uint32_t sn;     // 12-bit or 18-bit sequence number
	uint8_t mac_i[PDCP_CONTROL_PLANE_DATA_PDU_MAC_I_SIZE];
} pdcp_user_plane_data_pdu_header;

/*
 * 6.2.2.2 User Plane PDCP Data PDU with long PDCP SN (12-bit)
 */
typedef struct {
	uint8_t dc;      // Data or control (see 6.3.7)
	uint32_t sn;     // 12-bit sequence number
} pdcp_user_plane_data_pdu_header_with_sn12;
#define PDCP_USER_PLANE_DATA_PDU_SN12_HEADER_SIZE 2
#define PDCP_USER_PLANE_DATA_PDU_MAC_I_SIZE 4

/*
 * 6.2.2.3 User Plane PDCP Data PDU with short PDCP SN (18-bit)
 */
typedef struct {
	uint8_t dc;
	uint32_t sn;      // 18-bit sequence number
} pdcp_user_plane_data_pdu_header_with_sn18;
#define PDCP_USER_PLANE_DATA_PDU_SN18_HEADER_SIZE 3
...
typedef struct {
	uint8_t dc;
	uint8_t pdu_type; // PDU type (see 6.3.8)
	uint16_t first_missing_count; // First missing PDCP COUNT
 ...
 */
#define PDCP_CONTROL_PDU_STATUS_REPORT_HEADER_SIZE 5
